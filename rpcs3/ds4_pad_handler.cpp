...
std::array<bool, MAX_GAMEPADS> ds4_pad_handler::GetConnectedControllers()
{
...
std::array<std::array<u8, 64>, MAX_GAMEPADS> ds4_pad_handler::GetControllerData()
{
...
std::vector<std::string> ds4_pad_handler::ListDevices()
{
	...
	{
		Init();
		if (!is_init) return ds4_pads_list;
	}
